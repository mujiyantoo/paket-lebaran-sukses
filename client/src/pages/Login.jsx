    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 shadow-2xl border border-white/10">
          ...
          <form onSubmit={(e) => { e.preventDefault(); alert('Demo mode - Login berfungsi dengan database'); }}>
            <div className="mb-6">
              <label className="block text-slate-300 text-sm font-medium mb-2">Email</label>
              <input 
                type="email" 
                placeholder="admin@paketlebaran.com"
            ...
            <button 
              type="submit" 
              className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold py-4 rounded-xl hover:shadow-lg hover:shadow-amber-500/30 transition-all"
            >
              Masuk Sekarang
            </button>
          </form>
          <p className="text-center text-slate-400 mt-6 text-sm">
            Belum punya akun? <Link to="/register" className="text-amber-400 hover:text-amber-300 font-medium">Daftar</Link>
          </p>
        </div>
        
        <p className="text-center text-slate-500 text-sm mt-6">
          © 2024 Paket Lebaran Sukses. All rights reserved.
        </p>
      </div>
    </div>
  );
